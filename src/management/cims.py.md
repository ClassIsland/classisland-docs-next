...
此群为CIMS.py的交流群，在此群发生的任何事情与 ClassIsland 开发者无关，ClassIsland 开发者不对其负责

:::

CIMS.py集控服务器分为三个部分，分别是`api` `command` `gRPC`，分别用于：

| 组件 | `api`   | `command` | `gRPC` |
|----|--------------------------------------|--------------------------------------------|--------------------------------------|
| 用途 | 向客户端分发配置文件                           | 通过API以集控服务器为中介获取客户端状态、向客户端发送指令             | 与客户端建立gRPC链接                         |
| 端口 | 50050 | 50052       | 50051                                |
...
目前所有的`README Guide`和`start.py`都是在 Windows 环境开发的，在 Linux 环境下已经有生产部署的先例，但在出现意料之外的问题时，还请在 [commit](https://github.com/MINIOpenSource/CIMS-backend) 中提供更多诊断信息，谢谢！
